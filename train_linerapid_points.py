...
from data_utils.rapidata_drawing import LineRapidDataLoader
...
def main(args):
    ...
    train_dataset = LineRapidDataLoader.concat_dataset_from_root(os.path.join(data_path, 'train'))
    test_dataset = LineRapidDataLoader.concat_dataset_from_root(os.path.join(data_path, 'val'))
    ...
    criterion = module.get_loss(mat_diff_loss_scale=0) #TODO
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            batch_losses = test(model.eval(), testDataLoader, criterion)
            ...
            log_string(f'Current Test Loss: {float(mean_loss)}. (Best : {best_loss})')
